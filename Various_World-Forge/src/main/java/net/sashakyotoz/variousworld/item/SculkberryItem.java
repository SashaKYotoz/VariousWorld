...
public class SculkberryItem extends Item {
	...
	private void execute(LevelAccessor world, double x, double y, double z) {
		if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == VWBlocks.SCULK_GRASS.get()) {
			if (Math.random() < 0.5) {
				world.setBlock(BlockPos.containing(x, y + 1, z), VWBlocks.SMALL_SCULK_BUSH.get().defaultBlockState(), 3);
			}
		}
	}
}
