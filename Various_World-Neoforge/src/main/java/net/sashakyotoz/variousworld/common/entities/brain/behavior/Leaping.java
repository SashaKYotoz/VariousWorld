...
public class Leaping extends Behavior<SquealingSpiderEntity> {
    ...
    private final TargetingConditions ramTargeting = TargetingConditions.forCombat().selector((living, serverLevel) -> !living.getType().equals(VWEntities.SQUEALING_SPIDER.get()) && living.level().getWorldBorder().isWithinBounds(living.getBoundingBox()));
    ...
    protected void tick(ServerLevel level, SquealingSpiderEntity owner, long gameTime) {
        ...
        if (!list.isEmpty()) {
            ...
            float f2 = isDamageSourceBlocked(level.damageSources().mobAttack(owner), owner) ? 0.5F : 1.0F;
    ...
    public boolean isDamageSourceBlocked(DamageSource damageSource, LivingEntity livingentity) {
        ...
        if (!damageSource.is(DamageTypeTags.BYPASSES_SHIELD) && livingentity.isBlocking() && !flag) {
            Vec3 vec32 = damageSource.getSourcePosition();
            if (vec32 != null) {
                Vec3 vec3 = livingentity.calculateViewVector(0.0F, livingentity.getYHeadRot());
                Vec3 vec31 = vec32.vectorTo(livingentity.position());
                vec31 = (new Vec3(vec31.x, 0.0F, vec31.z)).normalize();
                return vec31.dot(vec3) < (double) 0.0F;
            }
        }
        return false;
    }
