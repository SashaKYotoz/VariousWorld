...
//        for (Map.Entry<Item, ModelTemplate> entry : VWRegistryHelper.ITEM_MODELS.entrySet())
//            this.reg(entry.getKey(), entry.getValue());
    }

    private ItemModelBuilder saplingItem(DeferredBlock<Block> item) {
        return withExistingParent(item.getId().getPath(),
                ResourceLocation.parse("item/generated")).texture("layer0",
                ResourceLocation.fromNamespaceAndPath(VariousWorld.MOD_ID,"block/" + item.getId().getPath()));
    }
