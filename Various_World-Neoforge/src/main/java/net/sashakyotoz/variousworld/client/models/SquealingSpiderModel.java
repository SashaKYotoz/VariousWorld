...
public class SquealingSpiderModel<T extends SquealingSpiderEntity> extends HierarchicalModel<T> {
    ...
    @Override
    public void setupAnim(SquealingSpiderEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        animateWalk(SquealingSpiderAnimations.WALK, limbSwing, limbSwingAmount, 4f, 4);
        animate(entity.inhale, SquealingSpiderAnimations.ATTACK, ageInTicks, -1);
        animate(entity.shoot, SquealingSpiderAnimations.ATTACK, ageInTicks, 0.5f);
        animate(entity.slide, SquealingSpiderAnimations.JUMP, ageInTicks, -0.5f);
        animate(entity.slideBack, SquealingSpiderAnimations.JUMP, ageInTicks, -1f);
        animate(entity.longJump, SquealingSpiderAnimations.JUMP, ageInTicks, 0.5f);
    }

    @Override
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, int color) {
        head.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        small_body.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        belly.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        rear_leg.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        rear_leg1.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        middle_leg.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        middle_leg1.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        front_leg.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
        front_leg1.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
    }
